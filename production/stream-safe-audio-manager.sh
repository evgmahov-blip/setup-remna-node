...
# Health/history are always same-origin static JSON.
...
# Catalog may provide a relative stream_url. Never require an external origin.
...
# Repair the known uniquify-theme collision if this live page was generated by
# an older manager. Only repair the dynamic class expression, never arbitrary JS.
...
# Same for the listeners sorting token: data-sort is intentionally stable.
  ...
  jq -n --argjson generated "$(date +%s)" '
    {
      ...
      mounts: {
        "/radio-book": {
          status: "Active",
          listener_count: 0,
          format: {bitrate: 256, content_type: "audio/mpeg"},
          metadata: {now_playing: "Радио Книга — прямой литературный эфир"},
          stream_url: "/audio/radio-book"
        },
        "/alice-wonderland": {
          status: "Active",
          listener_count: 0,
          format: {bitrate: 64, content_type: "audio/mpeg"},
          metadata: {now_playing: "Lewis Carroll — Alice in Wonderland, Chapter 1"},
          stream_url: "/audio/alice-wonderland-ch01.mp3"
        },
        "/sherlock-holmes": {
          status: "Active",
          listener_count: 0,
          format: {bitrate: 64, content_type: "audio/mpeg"},
          metadata: {now_playing: "Arthur Conan Doyle — A Scandal in Bohemia"},
          stream_url: "/audio/sherlock-scandal-in-bohemia.mp3"
        },
        "/beethoven-moonlight": {
          status: "Active",
          listener_count: 0,
          ...
          stream_url: "/audio/beethoven-moonlight.mp3"
        },
        "/chopin-nocturne": {
          status: "Active",
          listener_count: 0,
          ...
          stream_url: "/audio/chopin-nocturne.mp3"
        },
        "/bach-air": {
          status: "Active",
          listener_count: 0,
...
validate_staged_site(){
  ...
  ' "$root/data/streams.json" >/dev/null || return 1
  ...
  if grep -Eqi 'deepbeat|bookradio\.hostingradio\.ru|archive\.org|upload\.wikimedia\.org' "$root/index.html" "$root/data/streams.json"; then
    return 1
  fi
...
ensure_radio_book_proxy(){
  ...
  docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$NGINX_CONTAINER" \
    || { fail "nginx container не запущен: $NGINX_CONTAINER"; return 1; }
  docker exec "$NGINX_CONTAINER" test -r /etc/ssl/certs/ca-certificates.crt \
    || { fail 'В nginx container нет CA bundle; TLS verify отключать не буду'; return 1; }
...
blocks = list(server_re.finditer(s))
target = None
for m in blocks:
    block = m.group(0)
    if re.search(r'(?m)^\s*server_name\s+' + re.escape(domain) + r'\s*;', block):
        target = m
        break
if target is None:
...
publish_staged_site(){
  ...
  tmp="$WWW_DIR/.index.html.safe-audio.tmp"
  install -m 0644 "$staged/index.html" "$tmp"
  mv -f -- "$tmp" "$WWW_DIR/index.html"

  for f in streams.json history.json; do
    tmp="$WWW_DIR/data/.${f}.safe-audio.tmp"
    install -m 0644 "$staged/data/$f" "$tmp"
    mv -f -- "$tmp" "$WWW_DIR/data/$f"
  done
  ...
  for f in \
    ...
    chopin-nocturne.mp3 \
    bach-air.mp3; do
    tmp="$WWW_DIR/audio/.${f}.safe-audio.tmp"
    install -m 0644 "$staged/audio/$f" "$tmp"
    mv -f -- "$tmp" "$WWW_DIR/audio/$f"
  done
...
install_safe_audio(){
  ...
  cp -a -- "$WWW_DIR/index.html" "$staged/index.html"
  patch_frontend "$staged/index.html" || { fail 'Не удалось подготовить same-origin frontend'; return 1; }
  write_catalog "$staged/data/streams.json" || return 1
  ...
  for f in \
    ...
    chopin-nocturne.mp3 \
    bach-air.mp3; do
    cp -a -- "$CACHE_DIR/$f" "$staged/audio/$f"
  done

  validate_staged_site "$staged" || { fail 'Staged safe-audio site не прошёл fail-closed validation'; return 1; }
  ensure_radio_book_proxy || return 1
  publish_staged_site "$staged" || return 1
  write_source_manifest
...
status_safe_audio(){
  ...
  if [[ -s "$WWW_DIR/data/streams.json" ]] && jq -e '(.mounts|length)==6 and ([.mounts[].stream_url|startswith("/audio/")]|all) and ([.mounts[].stream_url|contains("://")|not]|all)' "$WWW_DIR/data/streams.json" >/dev/null 2>&1; then
    echo '[OK] streams.json: 6 same-origin channels'
  else
    echo '[FAIL] streams.json'; bad=1
  fi
  if grep -Eqi 'deepbeat|bookradio\.hostingradio\.ru|archive\.org|upload\.wikimedia\.org' "$WWW_DIR/index.html" "$WWW_DIR/data/streams.json" 2>/dev/null; then
    echo '[FAIL] frontend/catalog содержит внешний origin'; bad=1
  else
    echo '[OK] frontend/catalog: external origins отсутствуют'
  fi
  ...
  elif grep -Fq "$MARK_BEGIN" "$NGINX_CONF" 2>/dev/null && grep -Fq 'proxy_ssl_verify on;' "$NGINX_CONF" 2>/dev/null; then
    echo '[OK] Radio Book same-origin nginx proxy configured, TLS verify=ON'
  else
    echo '[FAIL] Radio Book nginx proxy missing'; bad=1
  fi
  return "$bad"
}
