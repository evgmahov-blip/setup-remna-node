...
fetch_stream_audio(){
  ...
  if valid_stream_audio "$dst"; then
  ...
  if [[ -n "$STREAM_AUDIO_FIXTURE_DIR" ]]; then
    ...
    cp -f -- "$fixture" "$dst"
    valid_stream_audio "$dst" || { rm -f "$dst"; fail "STREAM fixture не прошёл validation: $name"; return 1; }
    return 0
  fi
  part="${dst}.part"
  rm -f -- "$part"
  ...
  if ! valid_stream_audio "$part"; then
...
validate_stream_runtime(){
  ...
  if command -v jq >/dev/null 2>&1; then
    jq -e '(.mounts|length)==6 and ([.mounts|keys[]|startswith("/audio/")]|all)' "$root/data/streams.json" >/dev/null || return 1
  fi
...
deploy_stream(){
  ...
  for f in \
    ...
    chopin-nocturne.mp3 \
    bach-air.mp3; do
    valid_stream_audio "$STREAM_AUDIO_CACHE/$f" || { rm -rf -- "$tmpdir"; fail "STREAM cache повреждён: $f"; return 1; }
    install -m 0644 "$STREAM_AUDIO_CACHE/$f" "$uq/audio/$f"
  done

  validate_stream_runtime "$uq" || { rm -rf -- "$tmpdir"; fail 'STREAM runtime validation после uniquify не пройдена'; return 1; }
  ...
  rm -rf -- "$tmpdir"
  finish_site stream
  log "[OK] SelfSteal сайт: STREAM (pinned $STREAM_REF, 6 LOCAL same-origin каналов)"
